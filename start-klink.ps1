# start-klink.ps1 — starts EVERYTHING for iPhone testing in one go.
# Opens 4 windows: backend, backend tunnel, metro tunnel, metro.
# Auto-wires the random tunnel URLs into the app and prints the QR to scan.
#
# BEFORE RUNNING: close any old terminal windows running the backend or "npm start".
# Run with:  powershell -ExecutionPolicy Bypass -File C:\Users\lemue\Desktop\KlinkApp\start-klink.ps1

$app = 'C:\Users\lemue\Desktop\KlinkApp'
$backendDir = 'C:\Users\lemue\Desktop\demo5'
...
Start-Process powershell -ArgumentList '-NoExit', '-Command', "Set-Location '$app'; Write-Host 'Backend tunnel running - see launcher window for URL. Do not close.'; npx cloudflared tunnel --url http://localhost:8080 *> '$btLog'"
Start-Process powershell -ArgumentList '-NoExit', '-Command', "Set-Location '$app'; Write-Host 'Metro tunnel running - see launcher window for URL. Do not close.'; npx cloudflared tunnel --url http://localhost:8081 *> '$mtLog'"
...
Write-Host "[3/4] Updating TUNNEL_API_URL in constants.ts..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "================= SCAN THIS WITH THE iPHONE CAMERA =================" -ForegroundColor Yellow
node -e "require('qrcode-terminal').generate('$expUrl', {small: true})"
Write-Host ""
Write-Host "Or type in Safari on the iPhone:  $expUrl" -ForegroundColor Yellow
Write-Host "(Wait ~2 min for the backend window to say 'Started Demo5Application' before logging in.)"
Write-Host ""
